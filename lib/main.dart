                        ...
                        children: [
                          Container(
                            ...
                            child: const Row(
                              ...
                              children: [
                                Icon(Icons.stars, color: Colors.indigoAccent, size: 16),
                                SizedBox(width: 6),
                                Text(
                                  'MINI GAME ARCADE',
                                  style: TextStyle(
                ...
                sliver: SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    _buildModernGameCard(
                      ...
                      tag: 'GAME 02',
                      title: '3초 반응속도 테스트',
                      subtitle: '화면 색상이 바뀔 때 한계의 순발력을 측정하세요',
                    ...
                    _buildModernGameCard(
                      ...
                      tag: 'GAME 03',
                      title: '1to25 순발력 측정',
                      subtitle: '1부터 25까지의 숫자를 빛의 속도로 터치!',
...
class _RabbitRunScreenState extends State<RabbitRunScreen> {
  ...
  void _shareResult() {
    final String text = '🐰 [토끼 탈출기] 최고 기록: $score점! 달린 시간: ${elapsedTime.toStringAsFixed(1)}초! 함께 도전해보세요!';
    Clipboard.setData(ClipboardData(text: text));
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('클립보드에 결과가 복사되었습니다!'),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 12),
                            Text('최종 점수: $score', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
                            Text('달린 시간: ${elapsedTime.toStringAsFixed(1)}초', style: TextStyle(fontSize: 14, color: Colors.white.withOpacity(0.7))),
...
class _ReactionTestScreenState extends State<ReactionTestScreen> {
  ...
  String _getGrade(int ms) {
    if (ms < 180) return '⚡ 신급 순발력!';
    if (ms < 240) return '🚀 초인적 반응!';
    if (ms < 300) return '👍 평균 이상!';
    return '🐢 조금 더 빠르게!';
  }

  void _shareResult() {
    if (_reactionTimeMs == null) return;
    final String text = '⚡ [반응속도 테스트] 내 반응속도: ${_reactionTimeMs}ms (${_getGrade(_reactionTimeMs!)})! 도전해보세요!';
    Clipboard.setData(ClipboardData(text: text));
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('클립보드에 결과가 복사되었습니다!'),
  ...
  Widget _buildContent() {
    if (_state == ReactionState.waiting) {
      return const Text(
        "터치하여 반응속도 테스트를 시작하세요",
        textAlign: TextAlign.center,
        style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold),
      );
    } else if (_state == ReactionState.ready) {
      return const Text(
        "초록색으로 바뀌면 즉시 터치하세요!",
        textAlign: TextAlign.center,
      ...
      return const Text(
        "너무 빨랐습니다!\n초록색이 된 후에 터치하세요.\n(다시 하려면 터치)",
        textAlign: TextAlign.center,
        style: TextStyle(fontSize: 22, color: Colors.white, fontWeight: FontWeight.bold, height: 1.4),
      );
    } else {
      if (_reactionTimeMs == null) {
        return const Text(
        ...
        return Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.bolt, color: Color(0xFF10B981), size: 54),
              const SizedBox(height: 12),
              Text(
                _getGrade(_reactionTimeMs!),
...
class _OneToTwentyFiveScreenState extends State<OneToTwentyFiveScreen> {
  ...
  void _shareResult() {
    final String text = '🎯 [1to25 순발력] 기록: $elapsedTime! 얼마나 빨라질 수 있는지 도전하세요!';
    Clipboard.setData(ClipboardData(text: text));
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('클립보드에 결과가 복사되었습니다!'),
          ...
          children: [
            Container(
              ...
              child: Row(
                ...
                children: [
                  Text('다음 숫자: $currentTarget', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                  Text('시간: $elapsedTime', style: const TextStyle(fontSize: 18, color: Color(0xFFA855F7), fontWeight: FontWeight.bold)),
                ],
              ),
            ),
            const SizedBox(height: 20),
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: isCleared ? Colors.white.withOpacity(0.05) : const Color(0xFFA855F7),
                        foregroundColor: Colors.white,
                        disabledBackgroundColor: Colors.white.withOpacity(0.02),
                        elevation: isCleared ? 0 : 4,
            ...
            if (isCompleted)
              Container(
                ...
                child: Column(
                  children: [
                    const Text('🎉 미션 완료!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF10B981))),
                    const SizedBox(height: 6),
                    Text('최종 기록: $elapsedTime', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.white)),
                    const SizedBox(height: 16),
                    Row(
...
// ==========================================
// GAME 4: Missile Escape
// ==========================================

const Size kPlayerRenderSize = Size(44, 54);
const double kPlayerHitboxRadius = 11.0;
const double kBulletRadius = 7.0;
const double kBulletHitboxRadius = 5.0;
const double kBulletTrailLength = 15.0;
...
const double kInvincibleBlinkStart = 0.8;

const bool kUseAssetFighter = false;
const String kFighterAssetPath = 'assets/fighterJet.png';
const Rect kFighterSourceRect = Rect.fromLTWH(394, 41, 492, 605);
const int kFighterBackgroundR = 18;
const int kFighterBackgroundG = 30;
const int kFighterBackgroundB = 49;
const int kFighterFloodColorDistance = 38;
...
class _MissileEscapeScreenState extends State<MissileEscapeScreen> {
  ...
  int _nextSpecialChargeScore = kSpecialStockScoreStep;
  double invincibleTimeRemaining = 0.0;

  ui.Image? fighterImage;
  bool _fighterAssetLoadFailed = false;
  ...
  Future<void> _loadFighterImage() async {
    try {
      final ByteData data = await rootBundle.load(kFighterAssetPath);
      final Uint8List encoded = data.buffer.asUint8List();
      final ui.Codec codec = await ui.instantiateImageCodec(encoded);
      final ui.FrameInfo frame = await codec.getNextFrame();
      final ui.Image source = frame.image;
      final ByteData? raw = await source.toByteData(format: ui.ImageByteFormat.rawRgba);
      if (raw == null) throw StateError('fighterJet.png RGBA 변환 실패');

      final Uint8List sourcePixels = raw.buffer.asUint8List();
      final int cropLeft = kFighterSourceRect.left.round();
      final int cropTop = kFighterSourceRect.top.round();
      final int cropWidth = kFighterSourceRect.width.round();
      final int cropHeight = kFighterSourceRect.height.round();
      final Uint8List cropPixels = Uint8List(cropWidth * cropHeight * 4);

      for (int y = 0; y < cropHeight; y++) {
        final int sourceStart = ((cropTop + y) * source.width + cropLeft) * 4;
        final int targetStart = y * cropWidth * 4;
        cropPixels.setRange(
          targetStart,
          targetStart + cropWidth * 4,
          sourcePixels,
          sourceStart,
        );
      }

      _floodFillConnectedBackground(cropPixels, cropWidth, cropHeight);
      ...
      final ui.Image cleaned = await completer.future;

      if (!mounted) return;
      setState(() {
        fighterImage = cleaned;
        _fighterAssetLoadFailed = false;
      });
    } catch (e) {
      debugPrint('전투기 에셋 처리 실패 - 벡터 전투기로 대체: $e');
      if (!mounted) return;
      setState(() {
        fighterImage = null;
        _fighterAssetLoadFailed = true;
      });
    }
  }

  void _floodFillConnectedBackground(Uint8List pixels, int width, int height) {
    final Uint8List visited = Uint8List(width * height);
    final List<int> queue = <int>[];
    int head = 0;

    bool isBackground(int pixelIndex) {
      final int p = pixelIndex * 4;
      final int dr = pixels[p] - kFighterBackgroundR;
      final int dg = pixels[p + 1] - kFighterBackgroundG;
      final int db = pixels[p + 2] - kFighterBackgroundB;
      return dr * dr + dg * dg + db * db <=
          kFighterFloodColorDistance * kFighterFloodColorDistance;
    }
    ...
    for (int i = 0; i < 7; i++) {
      clouds.add(
        CloudParticle(
          ...
          scale: 0.55 + _random.nextDouble() * 0.7,
          speed: 22.0 + _random.nextDouble() * 38.0,
          opacity: 0.07 + _random.nextDouble() * 0.10,
        ),
      );
    }
  }
  ...
  MissileEscapeDifficulty _getDifficulty(double t) {
    final double spawnInterval = max(0.10, 1.15 * exp(-t / 38.0));
    final int maxMissiles = min(100, 10 + (t * 1.10).round());
    final double missileSpeed = min(430.0, 150.0 + t * 3.0);
    final int burstCount = 1 + (t / 45.0).floor().clamp(0, 2);
  ...
  void _startGame() {
    ...
    setState(() {
      ...
      specialStocks = 0;
      specialProgress = 0.0;
      _nextSpecialChargeScore = kSpecialStockScoreStep;
      ...
      if (screenSize != Size.zero) {
        playerPosition = Offset(screenSize.width / 2, screenSize.height * 0.65);
      }
    });
  ...
  void _updateGame(double dt) {
    ...
    setState(() {
      ...
      while (score >= _nextSpecialChargeScore) {
        if (specialStocks < kMaxSpecialStock) specialStocks++;
        _nextSpecialChargeScore += kSpecialStockScoreStep;
      }
      specialProgress = specialStocks >= kMaxSpecialStock
          ? 1.0
          : (score % kSpecialStockScoreStep) / kSpecialStockScoreStep;

      if (invincibleTimeRemaining > 0.0) {
        invincibleTimeRemaining = max(0.0, invincibleTimeRemaining - dt);
      }

      if (joystickDir != Offset.zero) {
        final Offset next = playerPosition + joystickDir * kPlayerMaxSpeed * dt;
      ...
      for (final CloudParticle cloud in clouds) {
        cloud.position = Offset(cloud.position.dx, cloud.position.dy + cloud.speed * dt);
        if (cloud.position.dy > screenSize.height + 70) {
          cloud.position = Offset(_random.nextDouble() * screenSize.width, -70);
        }
      }

      for (int i = shockwaves.length - 1; i >= 0; i--) {
        final Shockwave sw = shockwaves[i];
        sw.radius += dt * 800.0;
        sw.opacity = max(0.0, 1.0 - sw.radius / sw.maxRadius);
        if (sw.radius >= sw.maxRadius) shockwaves.removeAt(i);
      }

      timeSinceLastSpawn += dt;
      final MissileEscapeDifficulty difficulty = _getDifficulty(elapsedTime);
      if (timeSinceLastSpawn >= difficulty.spawnInterval &&
        ...
        for (int i = 0; i < difficulty.burstCount; i++) {
          if (missiles.length >= difficulty.maxMissiles) break;
          _spawnMissile(difficulty.missileSpeed);
        }
      }

      for (int i = missiles.length - 1; i >= 0; i--) {
        final Missile missile = missiles[i];
        missile.position += missile.velocity * dt;

        if (missile.position.dx < -60 ||
            missile.position.dx > screenSize.width + 60 ||
            missile.position.dy < -60 ||
            missile.position.dy > screenSize.height + 60) {
          missiles.removeAt(i);
          continue;
        }

        if (invincibleTimeRemaining <= 0.0 && _checkCollision(missile)) {
          playerWasHit = true;
          break;
        }
      }
  ...
  void _spawnMissile(double speed) {
    ...
    switch (edge) {
      case 0:
        start = Offset(_random.nextDouble() * screenSize.width, -20);
      case 1:
        start = Offset(_random.nextDouble() * screenSize.width, screenSize.height + 20);
      case 2:
        start = Offset(-20, _random.nextDouble() * screenSize.height);
      case 3:
        start = Offset(screenSize.width + 20, _random.nextDouble() * screenSize.height);
    }

    final Offset delta = playerPosition - start;
    if (delta.distanceSquared == 0) return;
    final Offset velocity = delta / delta.distance * speed;
    missiles.add(Missile(position: start, velocity: velocity));
  }

  bool _checkCollision(Missile missile) {
    final double minDistance = kPlayerHitboxRadius + kBulletHitboxRadius;
    return (playerPosition - missile.position).distanceSquared <=
    ...
    setState(() {
      ...
      shockwaves.add(
        Shockwave(
          ...
          radius: 10.0,
          maxRadius: max(screenSize.width, screenSize.height) * 1.15,
        ),
      );
      missiles.clear();
    });
  ...
  void _shareResult() {
    final String text = '🚀 [Missile Escape] 최고 점수: $score점! (생존 시간: ${elapsedTime.toStringAsFixed(1)}초)! 탄막 피하기에 도전하세요!';
    Clipboard.setData(ClipboardData(text: text));
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('클립보드에 결과가 복사되었습니다!'),
  ...
  void _updateJoystick(Offset localPosition) {
    if (!isPlaying) return;
    final Offset center = const Offset(kJoystickBaseRadius, kJoystickBaseRadius);
    final Offset delta = localPosition - center;
    final double maxDistance = kJoystickBaseRadius - kJoystickStickRadius;
    final double distance = delta.distance;
    final Offset clamped = distance > maxDistance && distance > 0
        ? delta / distance * maxDistance
        : delta;

    setState(() {
      joystickStickPos = clamped;
      joystickDir = maxDistance == 0 ? Offset.zero : clamped / maxDistance;
    });
  }
              ...
              child: Transform.translate(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: Colors.white.withValues(alpha: 0.90), width: 2),
                  ),
                ),
              ),
            ),
  ...
  Widget _buildSpecialButton() {
    final bool canUse = specialStocks > 0;
          ...
          children: [
                    ...
                    child: BackdropFilter(
                      ...
                      child: Container(
                        ...
                        child: Icon(
                          ...
                          size: 36,
                          color: canUse ? const Color(0xFFFFD54F) : Colors.white38,
                        ),
                      ),
                    ),
                  ),
            ...
            Row(
              ...
              children: List.generate(kMaxSpecialStock, (index) {
                final bool active = index < specialStocks;
                return Container(
                  width: 14,
                  height: 6,
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final double horizontalControlPadding = screenSize.width < 430 ? 10.0 : 20.0;
          final double controlScale = screenSize.width < 430 ? 0.82 : 1.0;
              ...
              RepaintBoundary(
                child: CustomPaint(
                  painter: MissileGamePainter(
                    ...
                    useAssetFighter: kUseAssetFighter && !_fighterAssetLoadFailed,
                    ...
                    child: Row(
                      children: [
                        GestureDetector(
                          ...
                          child: _glassChip(
                            padding: const EdgeInsets.all(8),
                            child: const Icon(Icons.arrow_back, color: Colors.white, size: 21),
                          ),
                        ),
                        const Spacer(),
                        _glassChip(
                          child: Text(
                            'SCORE $score   ${elapsedTime.toStringAsFixed(1)}s',
                            style: const TextStyle(
                              fontSize: 17,
              ...
              if (isPlaying && !isGameOver) ...[
                SafeArea(
                  child: Align(
                    alignment: Alignment.bottomLeft,
                    child: Padding(
                      padding: EdgeInsets.only(left: horizontalControlPadding, bottom: 14),
                ...
                SafeArea(
                  child: Align(
                    alignment: Alignment.bottomRight,
                    child: Padding(
                      padding: EdgeInsets.only(right: horizontalControlPadding, bottom: 16),
                      child: Transform.scale(
                        scale: controlScale,
                        alignment: Alignment.bottomRight,
                  ...
                  SafeArea(
                    child: Align(
                      ...
                      child: Padding(
                        ...
                        child: _glassChip(
                          child: Text(
                            '무적 ${invincibleTimeRemaining.toStringAsFixed(1)}s',
                            style: const TextStyle(
                ...
                Center(
                  child: _glassChip(
                    ...
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        const Icon(Icons.airplanemode_active, size: 56, color: Color(0xFF80DEEA)),
                        const SizedBox(height: 10),
                        const Text(
                          'MISSILE ESCAPE',
                          style: TextStyle(
                        ...
                        ElevatedButton(
                          ...
                          child: const Text('게임 시작', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)),
                        ),
                      ],
                    ),
                  ),
                  ...
                  child: _glassChip(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 12),
                        Text('최종 점수: $score', style: const TextStyle(fontSize: 22, color: Colors.white, fontWeight: FontWeight.bold)),
                        Text(
                          '생존 시간: ${elapsedTime.toStringAsFixed(1)}초',
...
class MissileGamePainter extends CustomPainter {
  final ui.Image? fighterImage;
  final bool useAssetFighter;
  ...
  static final Paint _starPaint = Paint()..color = Colors.white.withValues(alpha: 0.60);
  static final Paint _cloudPaint = Paint();
  static final Paint _bulletOuterPaint = Paint()..color = const Color(0x55FF8A00);
  static final Paint _bulletBodyPaint = Paint()..color = const Color(0xFFFF9800);
  static final Paint _bulletCorePaint = Paint()..color = const Color(0xFFFFF3C4);
  static final Paint _bulletTrailPaint = Paint()
    ..color = const Color(0xCCFF6D00)
    ..strokeCap = StrokeCap.round;
  static final Paint _fighterOutlinePaint = Paint()
    ..color = const Color(0xFF4DD0E1)
    ..style = PaintingStyle.stroke
    ..strokeWidth = 2.2
    ..strokeJoin = StrokeJoin.round;
  static final Paint _fighterBodyPaint = Paint()..color = const Color(0xFFD7E3F0);
  static final Paint _fighterShadowPaint = Paint()..color = const Color(0xFF90A4AE);
  static final Paint _canopyPaint = Paint()..color = const Color(0xFF81D4FA);
  static final Paint _enginePaint = Paint()..color = const Color(0xFF80DEEA);

  MissileGamePainter({
    required this.fighterImage,
    required this.useAssetFighter,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final Shockwave sw in shockwaves) {
      final Paint paint = Paint()
        ..style = PaintingStyle.stroke
        ..strokeWidth = 5.0
        ..color = const Color(0xFF80DEEA).withValues(alpha: sw.opacity);
      canvas.drawCircle(sw.center, sw.radius, paint);
    }
    ...
    if (useAssetFighter && fighterImage != null) {
      _drawAssetFighter(canvas, playerPosition, playerRenderSize, fighterImage!);
    } else {
  ...
  void _drawBackground(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 34; i++) {
      final double x = ((i * 83.0) + 17.0) % size.width;
      final double y = ((i * 47.0) + 23.0) % (size.height * 0.78);
      canvas.drawCircle(Offset(x, y), i.isEven ? 1.0 : 1.5, _starPaint);
    }
  }

  void _drawCloud(Canvas canvas, Offset pos, double scale, double opacity) {
    _cloudPaint.color = const Color(0xFFE3F2FD).withValues(alpha: opacity);
    final double w = 88 * scale;
    final double h = 26 * scale;
    canvas.drawOval(Rect.fromCenter(center: pos, width: w, height: h), _cloudPaint);
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(pos.dx - 23 * scale, pos.dy - 7 * scale),
        width: 43 * scale,
        height: 29 * scale,
      ),
      _cloudPaint,
    ...
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(pos.dx + 19 * scale, pos.dy - 9 * scale),
        width: 50 * scale,
        height: 34 * scale,
      ),
      _cloudPaint,
  ...
  void _drawAssetFighter(Canvas canvas, Offset center, Size size, ui.Image image) {
    final Rect dst = Rect.fromCenter(center: center, width: size.width, height: size.height);
    final Rect src = Rect.fromLTWH(0, 0, image.width.toDouble(), image.height.toDouble());
    final Paint paint = Paint()
      ..filterQuality = FilterQuality.high
      ..colorFilter = const ColorFilter.matrix(<double>[
        1.28, 0.00, 0.00, 0.00, 24.0,
        0.00, 1.28, 0.00, 0.00, 24.0,
        0.00, 0.00, 1.32, 0.00, 30.0,
  ...
  void _drawVectorFighter(Canvas canvas, Offset center, Size renderSize) {
    ...
    final double w = renderSize.width;
    final double h = renderSize.height;

    final double pulse = 0.86 + 0.14 * sin(elapsedTime * 28.0);
    final Path flame = Path()
      ..moveTo(-w * 0.085, h * 0.36)
      ..lineTo(0, h * (0.50 + 0.08 * pulse))
      ..lineTo(w * 0.085, h * 0.36)
      ..close();
    canvas.drawPath(flame, _enginePaint);
    canvas.drawCircle(Offset.zero.translate(0, h * 0.39), 3.2, Paint()..color = Colors.white);

    final Path body = Path()
      ..moveTo(0, -h * 0.50)
      ..lineTo(w * 0.11, -h * 0.17)
      ...
      ..lineTo(-w * 0.11, -h * 0.17)
      ..close();
    canvas.drawPath(body, _fighterBodyPaint);

    final Path centerShade = Path()
      ..moveTo(0, -h * 0.46)
      ..lineTo(w * 0.07, h * 0.31)
      ..lineTo(0, h * 0.34)
      ..lineTo(-w * 0.07, h * 0.31)
      ..close();
    canvas.drawPath(centerShade, _fighterShadowPaint);
    canvas.drawPath(body, _fighterOutlinePaint);

    final Path canopy = Path()
      ..moveTo(0, -h * 0.30)
      ..quadraticBezierTo(w * 0.105, -h * 0.12, w * 0.055, h * 0.02)
      ..quadraticBezierTo(0, h * 0.075, -w * 0.055, h * 0.02)
      ..quadraticBezierTo(-w * 0.105, -h * 0.12, 0, -h * 0.30)
      ..close();
    canvas.drawPath(canopy, _canopyPaint);
    canvas.drawPath(
      canopy,
      Paint()
  ...
  void _drawBullet(Canvas canvas, Missile missile) {
    final Offset pos = missile.position;
    final double speed = missile.velocity.distance;
    if (speed > 0.0) {
      final Offset trailEnd = pos - missile.velocity / speed * kBulletTrailLength;
      _bulletTrailPaint.strokeWidth = 3.2;
      canvas.drawLine(pos, trailEnd, _bulletTrailPaint);
    }

    canvas.drawCircle(pos, kBulletRadius + 3.0, _bulletOuterPaint);
    canvas.drawCircle(pos, kBulletRadius, _bulletBodyPaint);
    canvas.drawCircle(pos, kBulletRadius * 0.46, _bulletCorePaint);
  }

  void _drawInvincibleShield(Canvas canvas, Offset center, double timeRemaining) {
    if (timeRemaining < kInvincibleBlinkStart &&
        ((timeRemaining * 14).floor().isEven)) {
      return;
    }

    final double pulse = 1.0 + 0.055 * sin(elapsedTime * 18.0);
    final double radius = kPlayerRenderSize.height * 0.60 * pulse;
    canvas.drawCircle(
      center,
      radius,
      Paint()..color = const Color(0xFF4DD0E1).withValues(alpha: 0.12),
    );
    canvas.drawCircle(
      center,
      radius,
      Paint()
        ..style = PaintingStyle.stroke
        ..strokeWidth = 3.2
        ..color = const Color(0xFF80DEEA).withValues(alpha: 0.90),
    );
  }

  @override
  bool shouldRepaint(covariant MissileGamePainter oldDelegate) => true;
}